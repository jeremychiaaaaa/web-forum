...
import { setUserID, setUserLikedPost, setUsername, setUserPic } from '../../redux/actions'
...
const AllPosts = () => {
    ...
    const {category,username,user_id,edit,userLikedPost} = useSelector(state=>state.userReducer)
    ...
    useEffect(()=>{
        ...
        axios.get('/api/v1/auth/logged_in',{withCredentials:true}).then(
            res=>{
                ...
                if(res.data.data){
                    ...
                    let liked_posts = temp.map((item)=>{
                        if(item.type === 'like'){
                            return {
                            post_id: item.attributes.post_id, like_id: item.id
                            }
                        }
        ...
        if(category === 'All'){
            fetch('/api/v1/posts.json').then(res=>{
            ...
            axios.get(`/api/v1/posts/category/${category}`).then(res=>{
                //this is to get all the posts from the database
   ...
   let final = posts.map((post,index)=>{
    ...
    const individualPostsByPopularity = final.sort((a,b)=>  b.likes.data.length - a.likes.data.length).map((item,index)=>{
  ...
  return (
        <Container>
            ...
            <Header>
                ...
                <span style={{fontSize:'1.5rem',paddingBottom:5, fontWeight:600,borderBottom:topHeaderCategory === 'Latest'? '3px solid #ff7f50' :'none',fontFamily:'Poppins',cursor:'pointer',
                color:topHeaderCategory==='Latest' ? 'black':'rgba(220,220,220,1)'
                 }}
                ...
                <span style={{fontSize:'1.5rem',paddingBottom:5, fontWeight:600,borderBottom:topHeaderCategory === 'Popular'? '3px solid #ff7f50':'none' ,fontFamily:'Poppins',cursor:'pointer',
                 color:topHeaderCategory==='Popular' ? 'black':'rgba(220,220,220,1)'
                }}
                ...
                <IoMdAdd 
                onClick={addPost}
                style={{padding:15, borderRadius:5, background:'#ff7f50', color:'white',cursor:'pointer',transform:'translateX(30px)'}}
                />
            ...
            : 
                ...
                posts.length === 0 
                ?
                <NoPosts>Unfortunately no posts related to {searchBarError === '' ? category : searchBarError}...</NoPosts>
                :
              (
