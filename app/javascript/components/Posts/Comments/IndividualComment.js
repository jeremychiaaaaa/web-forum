...
const IndividualComment = ({ userId,content,createdAt,
                             commentId,reload ,setReload,
                             allComments,post_id,
                             comment_likes,phone, comment_likes_data
                            }) => {
    ...
    const [localCommentLikeTargetComment,setLocalCommentLikeTargetComment] = useState(false)
   ...
   const [profileUrl,setProfileUrl] = useState()
    ...
    useEffect(()=>{
        ...
        if(user_id){
            axios.get(`/api/v1/registrations/${user_id}`).then(res=>{
                
                let temp = res.data.included
               
                let liked_comment = (temp.map((item)=>{
                    if(item.type==='like_comment'){
                        return {
                            comment_id: item.attributes.comment_id, like_comment_id:item.id
                        }
                    }
        ...
        setChildComment(allComments.filter(item=>item.attributes.parent_id === Number(commentId)))
        

        // get the number of replies for each comment from the backend server
        axios.get(`/api/v1/comments/replies/${commentId}`).then(res=>{
    ...
    const handleSubmit = () => {
        ...
        axios.patch(`/api/v1/comments/${commentId}`,data).then(res=>{
    ...
    const handleDelete = () => {
        axios.delete(`/api/v1/comments/${commentId}`).then(res=>{
    ...
    const handleSubmitReply = () => {
        ...
        if(username === ''){
            ...
            axios.post(`/api/v1/comments`,commentData,{withCredentials:true}).then(res=>{
                if(res.status===200){
                    setReplying(false)
                    setReload(true)
                }
    ...
    const handleLikeComment = () => {
        ...
        if(user_id){
            ...
            axios.post('/api/v1/like_comments',data,{withCredentials:true}).then(res=>{
            ...
            if(res.status===200){
                ...
                let filtered = localUserLikedComment.map((item)=>{
                    if(item.comment_id === Number(commentId) || item.comment_id === commentId){
                ...
                axios.delete(`/api/v1/like_comments/${like_comment_id}`).then(res=>{
                    let temp = localUserLikedComment
                    const deletedIndex = localUserLikedComment.findIndex(item => (item)=>(item.comment_id === Number(commentId)) ||item.comment_id === commentId)
  ...
  return (
    ...
    <CommentContainer edit={edit} phone={phone}>
        <HorizontalDiv>
            <Title>
                By @{userComment}
                <img src={profileUrl ? profileUrl : defaultImage} style={{width:30,height:30,borderRadius:15,objectFit:'cover'}} />
            </Title>
            <CreatedAt>
        ...
        <HorizontalDiv>
                ...
                <form style={{width:'100%'}} onSubmit={handleSubmit}>
                    <InputField>
                        <TextArea value={edited} name='edited' onChange={(e)=>setEdited(e.target.value)}/>
                    </InputField>
                </form>
                <div style={{display:'flex',gap:10,width:'90%',justifyContent:'center',marginTop:10}}>
            ...
            {/* This here makes sure that the edit and delete icon will only appear on posts that belong to a user */}
                    
                {
                    ...
                    <CiEdit strokeWidth={1} style={{cursor:'pointer',fontSize:'1.2rem'}}
                    onClick={()=>setEdit(true)}
                    />
                    <AiOutlineDelete style={{cursor:'pointer',fontSize:'1.2rem'}}
        ...
        <BottomSection>
            <IconWrapper>
                ...
                
                {
                 ...
                 localUserLikedComment.map(item=>item.comment_id).indexOf(Number(commentId)) !== -1 || localCommentLikeTargetComment
                 ? <FcLike style={{fontSize:'1.3rem',cursor:'pointer',marginBottom:3}}
                    onClick={handleLikeComment}
                    />
                 : <AiOutlineHeart style={{fontSize:'1.3rem',cursor:'pointer',}} 
                    onClick={handleLikeComment}
                    />
            ...
            <IconWrapper>
                ...
                <TfiComment style={{fontSize:'1.2rem',marginTop:3,cursor:'pointer'}} />
            </IconWrapper>
    ...
    
    {
        replying && (
            <InputField comment={newComment} >
                  <TextArea 
                  autoFocus
                  rows='5' placeholder='Add A Reply'
                  name='newComment'  value={newComment}
