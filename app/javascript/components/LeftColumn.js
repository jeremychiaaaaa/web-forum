...
const IndividualCategory = styled.div`
    ...
    background-color:  ${props=> (props.active ? 'rgba(220,220,220,0.5)':'white')};
    clip-path: ${props=> (props.active && 'polygon(82% 0, 100% 50%, 82% 100%, 0% 100%, 0 50%, 0% 0%)')};
...
const LeftColumn = () => {
    ...
    useEffect(()=>{
        ...
        axios.get('/api/v1/categories.json').then(res=>dispatch(setAllCategories(res.data.data))).catch(res=>console.log(res))
    },[])
  ...
  return (
    <LeftBar>
        ...
        <Categories>
            <IndividualCategory onClick={()=>fetchCategoryPosts(-1,'All')} active={selected===-1 ? true : false}>
                <Logo src='https://cdn-icons-png.flaticon.com/512/1946/1946488.png'/>
                <span style={{color:selected=== -1 ? '#1e90ff':'gray',fontFamily:'Lato'}} >Home</span>
            </IndividualCategory>
            {allCategories.map((category,index)=>(
                <IndividualCategory onClick={()=>fetchCategoryPosts(index,category.attributes.name)} active={selected===index ? true : false} >
                    <Logo src={category.attributes.icon_url} />
                    <span style={{color:selected===index ? '#1e90ff':'gray',fontFamily:'Lato'}}>
