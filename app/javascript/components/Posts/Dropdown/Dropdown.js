...
const Dropdown = ({allCategories,categoryChoose,setCategoryChoose,categoryIDChoose,setCategoryIDChoose,phone}) => {
    ...
    <div className='dropdown'>
       ...
       (
        <div className='dropdown-content'>
            {allCategories.filter((i,index)=>index<=5).map((item,index)=>

                <div className='dropdown-item' onClick={()=>selectItem(item.name,item.id)} >
                    {item.name}
                </div>
            )}
