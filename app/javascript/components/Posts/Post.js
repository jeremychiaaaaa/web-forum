...
const Post = ({created_at, category_name,
               description,image_url,title,
               users,likes,comments,post_id,  
               slug, userPosts, phone,
               topHeaderCategory, 
               included, userId
                }) => {
  ...
  return (
    ...
    <Container onClick={redirectToViewPost} phone={phone} >
            ...
            <CreatedAt phone={phone}>Posted <Moment fromNow >{created_at}</Moment></CreatedAt>
            <Title phone={phone} style={{whiteSpace:'nowrap', overflow:'hidden',textOverflow:'ellipsis'}}>{title}</Title>
    ...
    <span style={{fontFamily:'Poppins',fontWeight:300, fontSize:phone ? '0.7rem' : '0.8rem',display:'flex',gap:phone ? 5 : 10,alignItems:'center'}}>
        @{users}
        <img src={profilePic ? profilePic : defaultImage} style={{width:phone ? 20 : 30,height:phone ? 20 : 30,borderRadius:phone ? 10 : 15,objectFit:'cover'}} />
    </span>
                    
        
        
    <div style={{display:'flex', gap:5,paddingBottom:phone ? 10 : 5}}>
           
           <div style={{display:'flex', justifyContent:'center',alignItems:'center',gap:3,pointerEvents:'all'}}>
           <span style={{fontSize:phone ? '1rem' : '1.2rem',}}>{localLikeCount}</span>
          
           {
            ...
            :  <AiOutlineHeart style={{fontSize:phone ? '1.2rem' : '1.3rem',cursor:'pointer'}} onClick={()=>likeButtonFunctionality(post_id)}/>
           }
          ...
          <div style={{display:'flex', justifyContent:'center',alignItems:'center',gap:3}}>
           <span style={{fontSize:phone ? '1rem' : '1.2rem'}}>{comments.data.length}</span>
           <TfiComment />
          </div>
