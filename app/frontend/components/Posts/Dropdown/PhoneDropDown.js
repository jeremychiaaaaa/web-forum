    ...
    <div className="dropdown">
      ...
      {active && (
        <div className="dropdown-content" style={{ zIndex: 20 }}>
          <div
            className={
              tempCategory === "All" ? "dropdown-item-active" : "dropdown-item"
            }
            onClick={() => phoneSelectCategory(-1, "All")}
          >
            All
          </div>
          {allCategories.map((item, index) => (
            <div
              className={
                tempCategory === item.attributes.name
                  ? "dropdown-item-active"
                  : "dropdown-item"
              }
              onClick={() => phoneSelectCategory(item.id, item.attributes.name)}
            >
